<x-app-layout>
    <div x-data="{ search: '' }" class="min-h-screen">
        ...
        <div class="overflow-x-auto p-4 text-black">
            <table class="min-w-full table-auto border-collapse border border-[#385c35]">
                ...
                <tr class="bg-gray-200 text-black text-center">
                    ...
                    <th class="px-4 py-2 border border-[#385c35]">Market Name</th>
                    <th class="px-4 py-2 text-center border border-[#385c35]">Actions</th>
                </tr>
                </thead>
                <tbody>
                <!-- Filtered Rows Based on Search -->
                @foreach($markets as $index => $item)
                        ...
                        <tr class="border-b text-center">
                            ...
                            <td class="px-4 py-2 text-center border border-[#385c35]">
                                <div class="flex justify-center space-x-2">
                                    <!-- Edit Button -->
                                    <a href="{{ route('market.show', $item->id) }}"
                                       class="text-blue-500 hover:underline">
                                        <x-heroicon-c-pencil-square class="w-5 h-5"/>
                                    </a>

                                    <!-- Delete Button -->
                                    <form method="POST" action="{{ route('market.destroy', $item->id) }}"
        ...
        <div class="mt-4 flex justify-between items-center ">
            <span>
                Showing {{ ($markets->currentPage() - 1) * $markets->perPage() + 1 }} -
                {{ min($markets->currentPage() * $markets->perPage(), $markets->total()) }} of {{ $markets->total() }} records
            </span>
            <div class="flex space-x-2">
                @if ($markets->currentPage() > 1)
                    <a href="{{ $markets->previousPageUrl() }}" class="px-4 py-2 bg-gray-300 text-black rounded">Previous</a>
                @endif
                @if ($markets->hasMorePages())
                    <a href="{{ $markets->nextPageUrl() }}" class="px-4 py-2 bg-gray-300 text-black rounded">Next</a>
                @endif
            </div>
        </div>
    </div>
