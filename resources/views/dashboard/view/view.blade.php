    ...
    <div x-data="{ search: '', openModal: false, selectedProduct: null }" class="min-h-screen">
        ...
        <div class="flex justify-end p-4">
            <input
                type="text"
                x-model="search"
                class="px-4 py-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white"
        ...
        <div class="overflow-x-auto p-4 dark:text-white">
            <table class="min-w-full table-auto border-collapse border border-gray-300 dark:border-gray-700">
                <thead>
                <tr class="bg-gray-200 dark:bg-gray-800 text-black dark:text-white text-center">
                    <th class="px-4 py-2 border dark:border-gray-700">#</th>
                    <th class="px-4 py-2 border dark:border-gray-700">Name</th>
                    <th class="px-4 py-2 border dark:border-gray-700">Trend</th>
                    <th class="px-4 py-2 border dark:border-gray-700">Change %</th>
                    <th class="px-4 py-2 text-center border dark:border-gray-700">Actions</th>
                </tr>
                </thead>
                ...
                @if($products->count() > 0)
                    ...
                    @foreach($products as $index => $product)
                        ...
                        @endphp
                        <template x-if="search === '' || '{{ $product->name }}'.toLowerCase().includes(search.toLowerCase())">
                            <tr class="border-b dark:border-gray-700 text-center">
                                <td class="px-4 py-2 border dark:border-gray-700">{{ ($products->currentPage() - 1) * $products->perPage() + $index + 1 }}</td>
        ...
        <div
            ...
            x-cloak
        >
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-lg w-full max-w-2xl p-6" @click.away="openModal = false">
                <div class="flex justify-between items-start mb-4">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white" x-text="selectedProduct ? selectedProduct.name : ''"></h2>
                ...
                <div class="mb-6">
                    <div class="flex items-center">
                        <span class="text-sm font-medium text-gray-500 dark:text-gray-400 mr-2">Price Trend:</span>
                            ...
                            <div class="flex items-center">
                                <template x-if="selectedProduct.price_trend === 'up'">
                                    <div class="flex items-center text-green-600 dark:text-green-400">
                                        ...
                                        <span x-text="'Increasing (' + selectedProduct.price_change_percent + '%)'"></span>
                                    </div>
                                </template>
                                <template x-if="selectedProduct.price_trend === 'down'">
                                    <div class="flex items-center text-red-600 dark:text-red-400">
                                        ...
                                        <span x-text="'Decreasing (' + selectedProduct.price_change_percent + '%)'"></span>
                                    </div>
                                </template>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Basic Information -->
                    <div class="space-y-4">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white border-b pb-2">Product Details</h3>
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Category</p>
                            <p class="text-gray-900 dark:text-white" x-text="selectedProduct?.category"></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Description</p>
                            <p class="text-gray-900 dark:text-white whitespace-pre-line" x-text="selectedProduct?.description"></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Measurement Unit</p>
                            <p class="text-gray-900 dark:text-white" x-text="selectedProduct?.measurement_unit"></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Seasonal Product</p>
                            <p class="text-gray-900 dark:text-white" x-text="selectedProduct?.seasonal ? 'Yes' : 'No'"></p>
                        </div>
                    </div>

                    <!-- Market Information -->
                    <div class="space-y-4">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white border-b pb-2">Market Data</h3>
                        <div>
                        ...
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Available Quantity</p>
                            <p class="text-gray-900 dark:text-white"
                               x-text="selectedProduct ? selectedProduct.quantity_available.toLocaleString('en-US') + ' ' + selectedProduct.measurement_unit : ''">
                            </p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Organic Certification</p>
                            <p class="text-gray-900 dark:text-white" x-text="selectedProduct?.is_organic ? 'Certified Organic' : 'Not Organic'"></p>
                        </div>
                    </div>
                </div>

                <div class="mt-6 flex justify-end">
                    <button @click="openModal = false" class="px-4 py-2 bg-gray-300 dark:bg-gray-700 rounded hover:bg-gray-400 dark:hover:bg-gray-600">
                        Close
                    </button>
                </div>
            </div>
        ...
        <!-- Pagination controls -->
        <div class="mt-4 flex justify-between items-center px-4 dark:text-white">
            <span>
            ...
            <div class="flex space-x-2">
                @if ($products->currentPage() > 1)
                    <a href="{{ $products->previousPageUrl() }}" class="px-4 py-2 bg-gray-300 dark:bg-gray-700 text-black dark:text-white rounded hover:bg-gray-400 dark:hover:bg-gray-600">
                        Previous
                    </a>
                @endif
                @if ($products->hasMorePages())
                    <a href="{{ $products->nextPageUrl() }}" class="px-4 py-2 bg-gray-300 dark:bg-gray-700 text-black dark:text-white rounded hover:bg-gray-400 dark:hover:bg-gray-600">
                        Next
                    </a>
                @endif
            </div>
