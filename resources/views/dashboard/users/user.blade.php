                    ...
                    <template x-if="search === '' || '{{ $item->name }}'.toLowerCase().includes(search.toLowerCase())">
                        <tr class="border-b text-center">
                            ...
                            <td class="px-4 py-2 border">
                                <form method="POST" action="{{ route('users.update-status', $item->id) }}" id="status-form-{{ $item->id }}">
                                    @csrf
                                    @method('PATCH')
                            ...
                            <td class="px-4 py-2 border">
                                <form method="POST" action="{{ route('users.update-role', $item->id) }}" id="role-form-{{ $item->id }}">
                                    @csrf
                                    @method('PATCH')
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input
                                        ...
                                        <span class="ml-3 text-sm font-medium">
                                            @if($item->role_id === 2) Admin @else User @endif
                                        </span>
                                    </label>
                                </form>
                            </td>
                            <td class="px-4 py-2 text-center border">
                                <div class="flex justify-center space-x-2">
                                    <!-- Delete Button -->
                                    <form method="POST" action="{{ route('user.destroy', $item->id) }}" onsubmit="return confirm('Are you sure you want to delete this user?')">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="mt-4 flex justify-between items-center dark:text-white">
            ...
            <div class="flex space-x-2">
                @if ($users->currentPage() > 1)
                    <a href="{{ $users->previousPageUrl() }}" class="px-4 py-2 bg-gray-300 text-black rounded">Previous</a>
                @endif
